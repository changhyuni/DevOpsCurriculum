...
HTTP/3은 TCP와 TLS에서 동작하는 HTTP/2의 단점으로 알려진 문제를 해결하고, TCP/IP가 아닌 QUIC이라는 UDP기반에 전송프로토콜 입니다. 이전 프로토콜인 HTTP/2를 살펴보면 주요기능인 멀티플렉싱을 사용하여 같은 TCP 연결을 통해 다수의 논리 스트림을 보낼 수 있고 많은 것을 더 좋고 빠르게 만들었습니다. 호스트와 연결되는 연결 숫자를 줄일 수 있고, DESHARDING이라는 기술을 사용하면 연결 수를 훨씬 더 줄일 수 있다. 하지만, TCP를 사용하면서 생기는 문제인 신뢰성이 높고 느리다 라는 제약 때문에 더 빠른 방법을 생각하면서 프로토콜 자체를 손보는 방법을 채택한 것이고, TCP로 먼저 구현할려 했으나 워낙 오래된 프로토콜이고 커널까지 내려가는 로우 레벨에서 정의되어 있기 때문에 수정하기 어려워 UDP를 선택한 것 입니다. TCP의 3-WAY-HADNSHARK에 대한 퍼포먼스도 문제가 되지만, HOLB(Head of Line Blocking)이라고 하는 문제도 존재했는데 HTTP 레벨에서의 HOLB와 TCP 레벨에서의 HOLB는 다른 의미이기는 하나 결국 어떤 요청에 병목이 생겨서 전체적인 레이턴시가 늘어난다는 맥락으로 본다면 동일하다고 볼 수 있습니다. TCP를 사용한 통신에서 패킷은 무조건 정확한 순서대로 처리되어야 합니다. 수신 측은 송신 측과 주고받은 시퀀스 번호를 참고하여 패킷을 재조립하기 때문인데, 그래서 통신 중간에 패킷이 손실되면 완전히 데이터로 다시 조립할 수 없기 때문에 절대로 그냥 넘어가지 않습니다. 무조건 송신 측은 수신 측이 패킷을 제대로 다 받았다는 것을 확인한 후, 만약 수신 측이 제대로 패킷을 받지 못했으면 해당 패킷을 다시 보내야 합니다. 또한, 패킷이 처리되는 순서 또한 정해져있으므로 이전에 받은 패킷을 파싱하기 전까지는 다음 패킷을 처리할 수도 없습니다. 이렇게 패킷이 중간에 유실되거나 수신 측의 패킷 파싱 속도가 느리다면 통신에 병목이 발생하게 되는 현상을 HOLB라고 부르는 것 입니다. 이것은 TCP 자체의 문제이므로 HTTP/1 뿐만 아니라 HTTP/2도 가지고 있는 문제입니다. 이러한 문제를 해결하기 위해 HTTP/3는 UDP 기반으로 만드러진 프로토콜인 QUIC 위에서 작동하는 것을 선택한 것입니다! 
